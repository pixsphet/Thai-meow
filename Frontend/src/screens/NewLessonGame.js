...
const NewLessonGame = ({ navigation, route }) => {
    const { lessonId = 1, category = 'consonants_basic' } = route.params || {};
    ...
    const loadVocabularyData = async () => {
        try {
            ...
            const lessonKey = category === 'basic' ? 'consonants_basic' : category;
            const vocabData = await vocabWordService.getVocabWordsForGame(lessonKey, 'matching', 10);
